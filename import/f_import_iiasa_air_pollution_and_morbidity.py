...
def main():  # pylint: disable=too-many-locals
    ...
    raw_air_pollution_factor = pd.read_excel(import_folder + "/air_pollutant_new.xlsx", engine="openpyxl")
    ...
    raw_morbidity_factor = pd.read_excel(import_folder + "/morbidity_new.xlsx", engine="openpyxl")
    # Rename columns
    ...
    for identifier, new_sectors in sector_value_mapping.items():
        replicates = raw_air_pollution_factor.loc[raw_air_pollution_factor["MICAT_SECTOR"] == identifier].copy()
        for index in new_sectors:
            subsector = id_subsector_table._data_frame.loc[index]["label"]
            replicates.loc[replicates["MICAT_SECTOR"] == identifier, "MICAT_SECTOR"] = subsector
            raw_air_pollution_factor = pd.concat([raw_air_pollution_factor, replicates])
            replicates.loc[replicates["MICAT_SECTOR"] == subsector, "MICAT_SECTOR"] = identifier
    for identifier, new_sectors in sector_value_mapping.items():
        replicates = raw_morbidity_factor.loc[raw_morbidity_factor["MICAT_SECTOR"] == identifier].copy()
        for index in new_sectors:
            subsector = id_subsector_table._data_frame.loc[index]["label"]
            replicates.loc[replicates["MICAT_SECTOR"] == identifier, "MICAT_SECTOR"] = subsector
    ...
    for identifier, new_carriers in carrier_value_mapping.items():
        replicates = raw_air_pollution_factor.loc[raw_air_pollution_factor["MICAT_FUEL"] == identifier].copy()
        for index in new_carriers:
            carrier = id_final_energy_carrier_table._data_frame.loc[index]["label"]
            replicates.loc[replicates["MICAT_FUEL"] == identifier, "MICAT_FUEL"] = carrier
            raw_air_pollution_factor = pd.concat([raw_air_pollution_factor, replicates])
            replicates.loc[replicates["MICAT_FUEL"] == carrier, "MICAT_FUEL"] = identifier
    for identifier, new_carriers in carrier_value_mapping.items():
        replicates = raw_morbidity_factor.loc[raw_morbidity_factor["MICAT_FUEL"] == identifier].copy()
        for index in new_carriers:
            carrier = id_final_energy_carrier_table._data_frame.loc[index]["label"]
            replicates.loc[replicates["MICAT_FUEL"] == identifier, "MICAT_FUEL"] = carrier
            raw_morbidity_factor = pd.concat([raw_morbidity_factor, replicates])
