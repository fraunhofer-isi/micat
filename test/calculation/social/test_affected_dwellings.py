...
import input.data_source  # pylint: disable=redefined-builtin
from calculation import extrapolation
from calculation.social import affected_dwellings
...
def _mocked_final_energy_saving_by_action_type():
    mocked_final_energy_saving_by_action_type = Mock()
    mocked_final_energy_saving_by_action_type.years = mocked_years
    return mocked_final_energy_saving_by_action_type
...
def test_determine_number_of_affected_dwellings():
    result = affected_dwellings.determine_number_of_affected_dwellings(
        _mocked_final_energy_saving_by_action_type(), _mocked_data_source(), 1
    )
...
@patch(input.data_source.DataSource.measure_specific_parameter, Mock(return_value='mocked_result'))
def test_user_input_parameter():
    result = affected_dwellings._user_input_parameter(
        'mocked_final_energy_saving_by_action_type', _mocked_data_source(), 'mocked_id_parameter'
    )
...
def test_number_of_affected_dwellings_per_ktoe():
    result = affected_dwellings._number_of_affected_dwellings_per_ktoe(
        _mocked_final_energy_saving_by_action_type(), _mocked_data_source()
    )
