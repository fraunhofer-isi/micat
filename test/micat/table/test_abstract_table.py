...
class TestPublicApi:
    ...
    class TestDel:
        def test_with_value_column(self):
            data_frame = pd.DataFrame(
                [{"id_measure": 1, "id_subsector": 1, "2000": 2, "2020": 3}]
            )
...
class TestPrivateApi:
    ...
    class TestCheckForDummyValuesAndRemoveThem:
        ...
        def test_contains_empty_value(self):
            with patch(
                AbstractTable._remove_rows_containing_value, "mocked_result"
            ) as mocked_remove:
                result = AbstractTable._check_for_special_values_and_remove_them(
                    "mocked_df", "mocked_dummy_value", "mocked_table_name"
                )
                assert result == "mocked_result"
    ...
    def test_column_names(self):
        ...
        id_column_names, year_column_names, value_column_names = (
            AbstractTable._column_names(data_frame)
        )
